...
name: federal-3115
description: >-
  Reviews Form 3115 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 3115
...
**Federal — Form 3115** in this repository's tax form change tracker.

## Scope

- Forms: Form 3115 (Application for Change in Accounting Method)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i3115
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i3115--{year}.pdf`
...
- Focus areas for this form: Accounting method changes. Continuous-use form — compare revision dates. Watch: automatic change procedure updates (the annual Rev. Proc. list-of-automatic-changes and Rev. Proc. 2025-28's Section 174A method changes), designated change numbers, audit-protection and Section 481(a) spread rules, and duplicate-copy filing addresses.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-3115/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-3115.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-3115.json`
