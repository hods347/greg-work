...
name: federal-8991
description: >-
  Reviews Form 8991 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8991
...
**Federal — Form 8991** in this repository's tax form change tracker.

## Scope

- Forms: Form 8991 (Tax on Base Erosion Payments of Taxpayers With Substantial Gross Receipts — BEAT)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8991
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8991--{year}.pdf`
...
- Focus areas for this form: BEAT return. Watch: the BEAT rate (permanently set at 10.5% for tax years beginning after 2025 under OBBBA, replacing the scheduled 12.5% increase), base-erosion percentage thresholds, treatment of credits in the BEAT computation, qualified derivative payment reporting, and aggregate-group determination rules.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8991/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8991.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8991.json`
