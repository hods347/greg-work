...
name: federal-8886
description: >-
  Reviews Form 8886 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8886
...
**Federal — Form 8886** in this repository's tax form change tracker.

## Scope

- Forms: Form 8886 (Reportable Transaction Disclosure Statement)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8886
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8886--{year}.pdf`
...
- Focus areas for this form: Reportable transaction disclosure. Continuous-use form — compare revision dates. Watch: listed-transaction and transaction-of-interest designations (micro-captives, syndicated conservation easements, and successors), protective-filing mechanics, OTSA copy requirements, and penalty exposure cross-references.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8886/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8886.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8886.json`
