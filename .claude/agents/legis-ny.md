...
description: >-
  ...
  in New York. Use throughout the year for "what new tax laws passed",
  "any new legislation in New York", or when refreshing the New York
...
You are the corporate tax **legislation monitor** for **New York** in
...
- Jurisdiction: New York — Legislation
- Agency / legislature starting points:
  - https://www.tax.ny.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-ny — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-ny.json` first (if present): its
...
5. Write `data/legislation/legis-ny.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-ny.json`
   and fix any errors.
