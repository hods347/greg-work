...
description: >-
  ...
  in West Virginia. Use throughout the year for "what new tax laws passed",
  "any new legislation in West Virginia", or when refreshing the West Virginia
...
You are the corporate tax **legislation monitor** for **West Virginia** in
...
- Jurisdiction: West Virginia — Legislation
- Agency / legislature starting points:
  - https://tax.wv.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-wv — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-wv.json` first (if present): its
...
5. Write `data/legislation/legis-wv.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-wv.json`
   and fix any errors.
