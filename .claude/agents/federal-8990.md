...
name: federal-8990
description: >-
  Reviews Form 8990 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8990
...
**Federal — Form 8990** in this repository's tax form change tracker.

## Scope

- Forms: Form 8990 (Limitation on Business Interest Expense Under Section 163(j))
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8990
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8990--{year}.pdf`
...
- Focus areas for this form: Section 163(j) limitation. Continuous-use form — compare revision dates. Watch: the ATI computation lines (EBITDA base restored for tax years beginning after 2024 under OBBBA), small-business exemption gross-receipts threshold indexing, excepted-trade election mechanics, partnership excess items (Schedules A/B), and CFC group election reporting.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8990/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8990.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8990.json`
