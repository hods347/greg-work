...
description: >-
  ...
  in Maryland. Use throughout the year for "what new tax laws passed",
  "any new legislation in Maryland", or when refreshing the Maryland
...
You are the corporate tax **legislation monitor** for **Maryland** in
...
- Jurisdiction: Maryland — Legislation
- Agency / legislature starting points:
  - https://www.marylandtaxes.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-md — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-md.json` first (if present): its
...
5. Write `data/legislation/legis-md.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-md.json`
   and fix any errors.
