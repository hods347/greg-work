...
name: federal-8858
description: >-
  Reviews Form 8858 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8858
...
**Federal — Form 8858** in this repository's tax form change tracker.
...
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8858
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8858--{year}.pdf`
...
- Focus areas for this form: FDE/FB information return. Watch: who-must-file expansions, Schedule C-1 (Section 987 gain/loss) changes — especially adoption of the final Section 987 regulations — Schedule J and M changes, dormant-entity relief, foreign branch category rules.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8858/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8858.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8858.json`
