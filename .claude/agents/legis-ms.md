...
description: >-
  ...
  in Mississippi. Use throughout the year for "what new tax laws passed",
  "any new legislation in Mississippi", or when refreshing the Mississippi
...
You are the corporate tax **legislation monitor** for **Mississippi** in
...
- Jurisdiction: Mississippi — Legislation
- Agency / legislature starting points:
  - https://www.dor.ms.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-ms — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-ms.json` first (if present): its
...
5. Write `data/legislation/legis-ms.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-ms.json`
   and fix any errors.
