...
name: federal-8975
description: >-
  Reviews Form 8975 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8975
...
**Federal — Form 8975** in this repository's tax form change tracker.

## Scope

- Forms: Form 8975 (Country-by-Country Report); Schedule A (Form 8975)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8975
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8975--{year}.pdf`
...
- Focus areas for this form: Country-by-country reporting for U.S. MNE groups with $850M+ revenue. Watch: OECD Pillar Two-driven definitional changes, constituent-entity and surrogate-filing rules, and any move toward public CbC alignment. Low churn historically but high stakes when it changes.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8975/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8975.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8975.json`
