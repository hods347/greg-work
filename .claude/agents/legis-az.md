...
description: >-
  ...
  in Arizona. Use throughout the year for "what new tax laws passed",
  "any new legislation in Arizona", or when refreshing the Arizona
...
You are the corporate tax **legislation monitor** for **Arizona** in
...
- Jurisdiction: Arizona — Legislation
- Agency / legislature starting points:
  - https://azdor.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-az — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-az.json` first (if present): its
...
5. Write `data/legislation/legis-az.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-az.json`
   and fix any errors.
