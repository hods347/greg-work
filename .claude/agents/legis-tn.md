...
description: >-
  ...
  in Tennessee. Use throughout the year for "what new tax laws passed",
  "any new legislation in Tennessee", or when refreshing the Tennessee
...
You are the corporate tax **legislation monitor** for **Tennessee** in
...
- Jurisdiction: Tennessee — Legislation
- Agency / legislature starting points:
  - https://www.tn.gov/revenue (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-tn — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-tn.json` first (if present): its
...
5. Write `data/legislation/legis-tn.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-tn.json`
   and fix any errors.
