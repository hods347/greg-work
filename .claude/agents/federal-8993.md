...
name: federal-8993
description: >-
  Reviews Form 8993 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8993
...
**Federal — Form 8993** in this repository's tax form change tracker.

## Scope

- Forms: Form 8993 (Section 250 Deduction for FDII/FDDEI and GILTI/NCTI)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8993
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8993--{year}.pdf`
...
- Focus areas for this form: Section 250 deduction. Watch: OBBBA changes for tax years beginning after 2025 — FDII becomes foreign-derived deduction eligible income (FDDEI), deduction percentages reset (roughly 33.34% FDDEI / 40% NCTI), QBAI-based deemed tangible income return removed from the computation — plus taxable-income limitation mechanics and expense allocation rules.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8993/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8993.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8993.json`
