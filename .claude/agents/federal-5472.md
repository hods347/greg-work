...
name: federal-5472
description: >-
  Reviews Form 5472 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 5472
...
**Federal — Form 5472** in this repository's tax form change tracker.
...
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i5472
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i5472--{year}.pdf`
...
- Focus areas for this form: Related-party transaction reporting for 25% foreign-owned U.S. corporations and foreign-owned disregarded entities. Watch: reportable-transaction category changes (Parts IV-VI), attribution rule changes (Section 318/958 interactions, including OBBBA's Section 958(b) changes), foreign-owned U.S. DE filing mechanics, penalty amounts ($25,000 base), and record-maintenance requirement changes.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-5472/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-5472.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-5472.json`
