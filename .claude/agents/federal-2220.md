...
name: federal-2220
description: >-
  Reviews Form 2220 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 2220
...
**Federal — Form 2220** in this repository's tax form change tracker.

## Scope

- Forms: Form 2220 (Underpayment of Estimated Tax by Corporations)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i2220
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i2220--{year}.pdf`
...
- Focus areas for this form: Estimated tax underpayment. Watch: annualization method changes, large-corporation rules, treatment of CAMT and BEAT in required installments (and any waiver relief notices), and recapture-type taxes excluded from the computation.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-2220/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-2220.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-2220.json`
