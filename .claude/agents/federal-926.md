...
name: federal-926
description: >-
  Reviews Form 926 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 926
...
**Federal — Form 926** in this repository's tax form change tracker.

## Scope

- Forms: Form 926 (Return by a U.S. Transferor of Property to a Foreign Corporation)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i926
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i926--{year}.pdf`
...
- Focus areas for this form: Outbound property transfers under Section 6038B. Continuous-use form — compare revision dates. Watch: Section 367 regulation changes (intangibles, Section 351/361 overlaps), cash-transfer reporting thresholds, gain recognition agreement cross-references, and penalty rules.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-926/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-926.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-926.json`
