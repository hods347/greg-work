...
name: federal-4562
description: >-
  Reviews Form 4562 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 4562
...
**Federal — Form 4562** in this repository's tax form change tracker.

## Scope

- Forms: Form 4562 (Depreciation and Amortization)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i4562
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i4562--{year}.pdf`
...
- Focus areas for this form: Depreciation and amortization. Watch: special depreciation allowance percentage and acquisition-date rules (100% restored for property acquired after 1/19/2025), Section 179 limits and SUV caps (indexed), new qualified production property (Section 168(n)) elections, amortization code changes for Section 174/174A, and listed-property thresholds.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-4562/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-4562.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-4562.json`
