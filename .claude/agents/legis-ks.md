...
description: >-
  ...
  in Kansas. Use throughout the year for "what new tax laws passed",
  "any new legislation in Kansas", or when refreshing the Kansas
...
You are the corporate tax **legislation monitor** for **Kansas** in
...
- Jurisdiction: Kansas — Legislation
- Agency / legislature starting points:
  - https://www.ksrevenue.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-ks — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-ks.json` first (if present): its
...
5. Write `data/legislation/legis-ks.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-ks.json`
   and fix any errors.
