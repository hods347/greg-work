...
description: >-
  ...
  in Wisconsin. Use throughout the year for "what new tax laws passed",
  "any new legislation in Wisconsin", or when refreshing the Wisconsin
...
You are the corporate tax **legislation monitor** for **Wisconsin** in
...
- Jurisdiction: Wisconsin — Legislation
- Agency / legislature starting points:
  - https://www.revenue.wi.gov (revenue agency news / law-change summaries)
...
- Related form-layer jurisdiction(s): state-wi — name the specific returns
  and schedules each law touches in `affected_returns`.

## Review window

Read the existing `data/legislation/legis-wi.json` first (if present): its
...
5. Write `data/legislation/legis-wi.json` per `schemas/legislation_entry.schema.json`,
   then run `python3 scripts/validate_changes.py data/legislation/legis-wi.json`
   and fix any errors.
