...
name: federal-8936
description: >-
  Reviews Form 8936 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8936
...
**Federal — Form 8936** in this repository's tax form change tracker.

## Scope

- Forms: Form 8936 (Clean Vehicle Credits); Schedule A (Form 8936)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8936
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8936--{year}.pdf`
...
- Focus areas for this form: Clean vehicle credits (Sections 30D, 25E, 45W commercial). Watch: OBBBA termination dates — no credit for vehicles acquired after September 30, 2025 — transition rules for binding contracts and placed-in-service timing, dealer transfer-election unwind, VIN reporting, and the 45W commercial clean vehicle computation corporations claim. Expect the instructions to shrink dramatically post-2025; confirm final-year claim mechanics.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8936/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8936.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8936.json`
