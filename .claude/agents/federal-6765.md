...
name: federal-6765
description: >-
  Reviews Form 6765 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 6765
...
**Federal — Form 6765** in this repository's tax form change tracker.

## Scope

- Forms: Form 6765 (Credit for Increasing Research Activities)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i6765
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i6765--{year}.pdf`
...
- Focus areas for this form: Research credit. High-churn: watch the new Section G business component detail reporting (phased mandatory disclosure of QRE detail per business component), Section E questions, the Section 280C reduced-credit election interacting with restored Section 174A expensing, payroll-tax offset mechanics for QSBs, and ASC vs regular credit computation changes.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-6765/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-6765.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-6765.json`
