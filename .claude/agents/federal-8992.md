...
name: federal-8992
description: >-
  Reviews Form 8992 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8992
...
**Federal — Form 8992** in this repository's tax form change tracker.

## Scope

- Forms: Form 8992 (U.S. Shareholder Calculation of Global Intangible Low-Taxed Income / Net CFC Tested Income)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8992
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8992--{year}.pdf`
...
- Focus areas for this form: GILTI/NCTI shareholder computation. Watch: the OBBBA overhaul for tax years beginning after 2025 — regime renamed net CFC tested income (NCTI), QBAI/net deemed tangible income return eliminated (expect Part I/II line removals), Schedule A changes, and consolidated-group (Schedule B) mechanics. Also watch tested-unit and high-tax exclusion regulation updates.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8992/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8992.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8992.json`
