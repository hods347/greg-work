...
name: federal-8865
description: >-
  Reviews Form 8865 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 8865
...
**Federal — Form 8865** in this repository's tax form change tracker.

## Scope

- Forms: Form 8865 (Return of U.S. Persons With Respect to Certain Foreign Partnerships)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i8865
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i8865--{year}.pdf`
...
- Focus areas for this form: Foreign partnership return. Watch: filer category changes, Schedules K-2/K-3 requirements, constructive ownership rules, Section 721(c) gain deferral reporting, Schedule O/P transfer reporting, alignment with Form 1065 changes.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-8865/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-8865.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-8865.json`
