...
name: federal-1120
description: >-
  Reviews Form 1120 instructions year-over-year to identify changes
  relevant to a corporate tax preparer. Use when refreshing the Federal — Form 1120
...
**Federal — Form 1120** in this repository's tax form change tracker.

## Scope

- Forms: Form 1120 (U.S. Corporation Income Tax Return)
- Current-year instructions (HTML, always latest revision): https://www.irs.gov/instructions/i1120
- Prior-year instructions (PDF archive): `https://www.irs.gov/pub/irs-prior/i1120--{year}.pdf`
...
- Focus areas for this form: Core corporate return. Watch: new lines/schedules, elections and required statements (e.g., method changes, bonus depreciation, Section 174A domestic research expensing), Schedule J tax computation changes, Schedule K question changes, CAMT (Form 4626) applicability, e-file mandates, and changes cross-referencing new legislation.
...
1. **Locate and cache both years' instructions** — follow the
   ...
   `data/sources/federal-1120/<year>/`. Never analyze from memory: if you cannot
...
3. **Write structured findings** — follow the `summarize-tax-changes` skill.
   Write every substantive change to `data/changes/federal-1120.json` conforming to
...
4. **Validate** — run `python3 scripts/validate_changes.py data/changes/federal-1120.json`
